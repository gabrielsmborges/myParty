...
def erasePastParties():
    ...
    codes = db.execute('SELECT code FROM parties WHERE date_hour < ?', dayafter)
    for el in codes:
...
def home():
    ...
    invitations = db.execute('SELECT * FROM guests WHERE guest_id = ?', session['user_id'])
    return render_template('home.html', name = f"{names['name']} {names['last_names']}", parties = parties, len_parties = len(parties), invitations = invitations,len_invitations = len(invitations), db = db)
...
def create():
    if request.method == "GET":
        ...
        db.execute('INSERT INTO parties (code, host_id, name, date_hour, address ,public, adult) VALUES (?, ?, ?, ?, ?, ?, ?)', hash_val, session["user_id"], party_name, formatted_date, address, public, adult)
...
@login_required
def myparties():
    parties = db.execute('SELECT * FROM parties WHERE host_id = ? ORDER BY date_hour DESC', session['user_id'])
...
def inviteuser():
    ...
    if request.method == "GET":
        ...
        username = request.form.get('username')
        guest_data = db.execute('SELECT * FROM users WHERE username = ?', username)
        #User is not found
        if len(guest_data) < 1:
        ...
        guest_birth = datetime.strptime(guest_data[0]['birth'], "%Y-%m-%d %H:%M:%S")
        guest_age = relativedelta(datetime.now(), guest_birth).years

        #If the user tries to invite a -18 person to a adult party
        if party_data[0]['adult'] == True and guest_age < 18:
            return f"{username} can't be invited because your party is for adult only."
        db.execute('INSERT INTO guests (party_code, guest_id, confirmed) VALUES (?, ?, ?)', code, guest_data[0]['id'], False)
...
def accept_invitation():
    ...
    if request.method == "GET":
        ...
        if action == "accept":
            ...
            db.execute('UPDATE guests SET confirmed = ? WHERE guest_id = ?', True, session['user_id'])
            db.execute('INSERT INTO bring (guest_id, party_id, element) VALUES (?, ?, ?)', session['user_id'], party_code, element)
            flash(f"Confirmed in {db.execute('SELECT name FROM parties WHERE code = ?' , party_code)[0]['name']}!")
            return redirect('/invitations')
        else:
            party_code = request.form.get('party_code')
            db.execute('DELETE FROM guests WHERE guest_id = ? AND party_code = ?', session['user_id'], party_code)
            flash(f"Removed from {db.execute('SELECT name FROM parties WHERE code = ?' , party_code)[0]['name']}!")
            return redirect('/invitations')
...
def search():
    ...
    if request.method == "GET":
        ...
        party_query = db.execute('SELECT * FROM parties WHERE code = ? AND public = ?', code, True)[0]
...
def publicparticipate():
    ...
    if request.method == "GET":
        ...
        birth = datetime.strptime(db.execute('SELECT birth FROM users WHERE id = ?', session['user_id'])[0]['birth'], "%Y-%m-%d %H:%M:%S")
        age = relativedelta(datetime.now(), birth).years
        data = json.loads(request.form.get('data').replace("'", '"'))
        if data['adult'] == True and age < 18:
            flash('This party is only for 18+ persons, you can not participate :(')
            return redirect('/search')
        bring = request.form.get('bring')
        db.execute('INSERT INTO guests (party_code, guest_id, confirmed) VALUES (?, ?, ?)', data['code'], session['user_id'], True)
        db.execute('INSERT INTO bring (guest_id, party_id, element) VALUES (?, ?, ?)', session['user_id'], data['code'], bring)
